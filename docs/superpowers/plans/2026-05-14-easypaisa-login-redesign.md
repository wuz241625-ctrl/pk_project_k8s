...
| 用例 | 步骤 | 结果 |
|---|---|---|
| H6 | 临时 wallet_status=0 → pre_login → second_login，验 Fix #1 不再 INVALID_TRANSITION | TBD（部署后填）|
| H6b | 跨 partner phone 防御纵深 | 单测覆盖，e2e 跳过 |
| H6c | URM90040 INCR 增量 + TTL 接近 3600 | TBD |
| H6d | session 不含 password 字段 | TBD |
| H6e | api.log smoke test 5 分钟无 ERROR | TBD |
