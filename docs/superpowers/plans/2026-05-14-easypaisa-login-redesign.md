...
| 5 | `f98b5fe5` → `735dfd1f` | `_verify_otp_fallback_chain` 完整 chain (upload+verifyFingerprint+secondLogin(with_pwd)+queryAccountList); 撤销原"Stage 1/Stage 2"两阶段设计 | urm90040.py: +2 测试; 顺便修 `_query_payment` 缺 `fingerprint_path` 字段 hidden bug |
...
|---|---|---|---|
| 二次上号正常 | 5-6s | < 2s | **5s** |
| verify_otp fallback (单一 chain, 含 verifyFingerprint + with_pwd 救冻) | 5-6s | 5-6s | 同步 (但 URM90040 救冻成功率↑) |
