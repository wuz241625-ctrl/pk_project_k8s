...
- 使用 per-size image_gen 源图集合生成 D7pay 品牌资产到 `ops/tenants/d7pay/assets/`，Android `192/144/96/72/48`、后台 `128`、下载页 `192`、favicon `256/64/48/32/16` 均有独立源图。
...
当前使用 image_gen 输出的多尺寸 D7 图标作为源图集合，生成脚本只做精确导出和 ICO 封装，不从单一主图裁切。后续如果用户提供原始 PNG/AI/SVG 文件，可以按同一尺寸集合替换源图并重新导出。
