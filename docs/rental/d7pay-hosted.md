...
D7pay launcher icon 使用独立资源 `@mipmap/ic_launcher_d7pay`，不覆盖默认 Ashrafi `ic_launcher`。admin、merchant、apkdownload 在 D7pay 构建模式下使用 D7pay logo 和 favicon，默认 Ashrafi 构建不受影响。
