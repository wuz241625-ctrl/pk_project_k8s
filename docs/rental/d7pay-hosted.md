...
- 运维唯一入口：`ops/tenants/d7pay/current-deployment-ops-runbook.md`
...
D7pay logo 唯一源图为 `ops/tenants/d7pay/assets/d7pay-logo-source-imagegen.png`，生成脚本会统一导出 Web、favicon、apkdownload 和 Android mipmap。D7pay launcher icon 使用独立资源 `@mipmap/ic_launcher_d7pay`，不覆盖默认 Ashrafi `ic_launcher`。admin、merchant、apkdownload 在 D7pay 构建模式下使用 D7pay logo 和 favicon，默认 Ashrafi 构建不受影响。
...
4. 使用 `APP_APPLICATION_ID=com.d7pay.merchant`、`APP_ICON=@mipmap/ic_launcher_d7pay` 和共享 release 签名构建 D7pay APK。
