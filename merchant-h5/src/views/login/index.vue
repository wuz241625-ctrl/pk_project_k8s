  ...
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" autocomplete="on" label-position="left">
      ...
      <div class="title-container">
        <h3 class="title">{{ $t('login.title', {  system: $env.VUE_APP_TITLE}) }}</h3>
      </div>
    ...
    <el-dialog title="$t('login.tip')" :visible.sync="dialogVisible" width="30%">
      <span>{{ message }}</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">{{$t('login.confirm')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (!validUsername(value)) {
        callback(new Error(this.$t('login.validation.username')))
      } else {
        callback()
      }
    }
    const validatePassword = (rule, value, callback) => {
      if (value.length < 6) {
        callback(new Error(this.$t('login.validation.password')))
      } else if (!isSafe(value)) {
        callback(new Error(this.$t('login.validation.passwordIllegal')))
      } else {
        callback()
      }
    }
    const validateGoogle = (rule, value, callback) => {
      if (value.length !== 6) {
        callback(new Error(this.$t('login.validation.googlecode')))
      } else if (isNaN(value) && !isFinite(value)) {
        callback(new Error(this.$t('login.validation.googlecodeInvalid')))
      } else {
        callback()
      }
    }
