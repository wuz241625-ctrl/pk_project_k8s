...
5. `ops/tenants/d7pay/k8s/go-worker-log-volumes.yaml` 必须先于 worker Deployment 应用；四个 worker 组件分别使用独立日志 PVC，避免 RWO PVC 被多个 Pod 跨节点共用。
6. 切流后，`api` Pod 内不得再启动 `pakistanpay_v2.py`、`Jazzcashpay_v2.py`、`auto_payout.py`、`jazzcash_auto_payout.py`、`jazzcash_monitor.py`、`notify.py`、`notify_df.py`、`time_out.py`。
...
- 当前 `d7pay-runtime-config` 明文 key：`RUN_ENV=PROD`、`TENANT_CODE=d7pay`、`MYSQL_HOST=mysql`、`MYSQL_DATABASE=pakistan_d7pay`、`REDIS_HOST=redis`、`API_CURRENT_PAY=pakistanpay`、`API_PAY_URL=https://api.d7pay.net/api/order/`、`API_OSPAY_API_HOST=https://api.d7pay.net/api`、`API_WEBSOCKET_ALLOW_HOST=api.d7pay.net`、`ADMIN_API_URL=http://api:9000`、`MERCHANT_API_URL=http://api:9000`、`BUSINESS_TIMEZONE=UTC`、`APP_DISPLAY_TIMEZONE=Asia/Karachi`。
- 当前 `d7pay-runtime-secret` 只在文档记录 key 名：`MYSQL_USER`、`MYSQL_PASSWORD`、`EASYPAISA_API_URL`、`EASYPAISA_USER_ID`、`EASYPAISA_SECRET_KEY`、`JAZZCASH_API_URL`、`JAZZCASH_USER_ID`、`JAZZCASH_SECRET_KEY`、`API_KEY_ORDER`、`API_SECRET_KEY`、`ADMIN_COOKIE_KEY`、`ADMIN_ID_TOKEN_KEY`、`MERCHANT_COOKIE_KEY`。
- Go worker 额外显式设置 `APP_MYSQL_DATABASE=pakistan_d7pay`、`APP_REDIS_ADDR=redis:6379`、`APP_REDIS_DB=0`、`ASYNQ_QUEUE=default` 和各组件自己的 `WORKER_ID`；实际 DSN 由环境拼为 `MYSQL_USER:MYSQL_PASSWORD@tcp(mysql:3306)/pakistan_d7pay?parseTime=true`。
...
```bash
cp ops/tenants/d7pay/k8s/app-secret.example.yaml /root/secrets/d7pay-runtime-secret.yaml
...
D7PAY_SECRET_YAML=/root/secrets/d7pay-runtime-secret.yaml
...
- `d7pay-runtime-config`、真实 `d7pay-runtime-secret` 存在。旧 `d7pay-config` / `d7pay-secret` 是未应用模板或历史文档命名，不作为当前线上验收对象。
