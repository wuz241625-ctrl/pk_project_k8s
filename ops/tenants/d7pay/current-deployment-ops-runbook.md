...
不存在 `admin-d7pay.awekay.com`、`merchant-d7pay.awekay.com`、`api-d7pay.awekay.com`、`apkdownload-d7pay.awekay.com`。
...
```text
apkdownload-d7pay 31080
admin-d7pay       31081
merchant-d7pay    31082
api-d7pay         31085
```

### 7. 配置 nginx 与 DNS

DNS 指向服务器公网 IP 后，在 `/etc/nginx/sites-enabled/pk` 增加 D7pay server block。
...
server {
    listen 80;
    server_name admin-d7pay.awekay.com;
...
server {
    listen 80;
    server_name merchant-d7pay.awekay.com;
...
server {
    listen 80;
    server_name api-d7pay.awekay.com;
...
server {
    listen 80;
    server_name apkdownload-d7pay.awekay.com;
...
如果 admin/merchant 开启 IP 白名单，D7pay 的 `admin-d7pay` 和 `merchant-d7pay` 要复制同一套白名单策略；不要把白名单加到 api 或 apkdownload，避免 App 和回调被误拦。
...
curl -I http://admin-d7pay.awekay.com/
curl -I http://merchant-d7pay.awekay.com/
curl -I http://apkdownload-d7pay.awekay.com/
curl -I http://api-d7pay.awekay.com/api/
```
...
- `admin-d7pay`、`merchant-d7pay`、`api-d7pay`、`apkdownload-d7pay` nginx 生效。
