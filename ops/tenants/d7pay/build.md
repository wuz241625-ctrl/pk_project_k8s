...
- `pk-go-worker`：从 `VANS8888/pk-go-worker` 的 `d7pay` 分支构建镜像；线上源码路径为 `/opt/cicd/k8s_d7pay/pk-go-worker`，GitHub 私库凭据只配置在服务器 `/root/.netrc` 和 `/var/lib/jenkins/.netrc`，不要写入脚本或仓库。发布到 `pk-d7pay` namespace 的四个独立 Deployment。
- `pk-go-worker` 日志：发布 worker 前必须先应用 `ops/tenants/d7pay/k8s/go-worker-log-volumes.yaml`。四个 worker 组件分别挂载独立 PVC 到 `/app/logs`，启动命令同时写 `kubectl logs` 和持久化文件。
- `api`：JCB 业务默认 `JAZZCASH_API_VERSION=v1.6`，当前线上未显式注入该 key 时走 `config.example.py` 默认值；不要改回代码内硬编码旧版本。
...
`api/admin/merchant` 和四个 `d7pay-go-worker*` Deployment 都通过以下对象注入运行时配置：

```yaml
envFrom:
  - configMapRef:
      name: d7pay-runtime-config
  - secretRef:
      name: d7pay-runtime-secret
```

当前 `d7pay-runtime-config` 明文 key：
...
REDIS_HOST=redis
API_CURRENT_PAY=pakistanpay
API_PAY_URL=https://api.d7pay.net/api/order/
API_OSPAY_API_HOST=https://api.d7pay.net/api
API_WEBSOCKET_ALLOW_HOST=api.d7pay.net
...
当前 `d7pay-runtime-secret` 只记录 key 名，不在文档保存明文值：
...
Go worker 额外显式设置 `APP_MYSQL_DATABASE=pakistan_d7pay`、`APP_REDIS_ADDR=redis:6379`、`APP_REDIS_DB=0`、`ASYNQ_QUEUE=default` 和各组件自己的 `WORKER_ID`。实际 MySQL DSN 由 Go worker 拼接为 `MYSQL_USER:MYSQL_PASSWORD@tcp(mysql:3306)/pakistan_d7pay?parseTime=true`。
