...
class EasyPaisaLegacyBridge:
    ...
    async def mirror_active(
        ...
        payment_key = keyspace.legacy_login_on_payment_key(payment_id)
        await self.redis.setex(payment_key, online_ttl, "1")
        if phone:
            phone_key = keyspace.legacy_login_on_phone_key(phone)
            await self.redis.setex(phone_key, online_ttl, "1")
        if dispatch_df:
...
class SyncEasyPaisaLegacyBridge:
    def __init__(self, redis):
        self.redis = redis
    ...
    def mirror_active(
        ...
        payment_key = keyspace.legacy_login_on_payment_key(payment_id)
        self.redis.setex(payment_key, online_ttl, "1")
        if phone:
            phone_key = keyspace.legacy_login_on_phone_key(phone)
            self.redis.setex(phone_key, online_ttl, "1")
        if dispatch_df:
