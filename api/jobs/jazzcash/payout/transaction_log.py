"""
JazzCash Transaction Logger — Operation logging to DB.
...
from jobs.common.logging_setup import TraceIDFilter

conf = get_config()


class TransactionLogger:
    """Handles operation logging and complete transaction recording."""

    def __init__(self, redis, logger, config, trace_id_filter=None):
        self.redis = redis
        self.logger = logger
        self.config = config
    ...
    def set_transfer_executor(self, transfer_executor):
        """Set reference to transfer executor for bank code lookups."""
        self._transfer_executor = transfer_executor

    def _is_pakistan_mobile_number(self, phone_number: str) -> bool:
        """Delegate to transfer_executor."""
        if hasattr(self, '_transfer_executor') and self._transfer_executor:
            return self._transfer_executor._is_pakistan_mobile_number(phone_number)
        # Fallback
        if not phone_number:
            return False
    ...
    def _convert_to_bank_code(self, raw_input: str) -> str:
        """Delegate to transfer_executor."""
        if hasattr(self, '_transfer_executor') and self._transfer_executor:
            return self._transfer_executor._convert_to_bank_code(raw_input)
        return raw_input or ''

    def _get_bank_name_by_ifsc(self, ifsc_code: str) -> str:
        """Delegate to transfer_executor."""
        if hasattr(self, '_transfer_executor') and self._transfer_executor:
            return self._transfer_executor._get_bank_name_by_ifsc(ifsc_code)
        return f'Bank ({ifsc_code})' if ifsc_code else None
