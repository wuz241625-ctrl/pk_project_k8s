...
def setup_high_performance_logging(use_async=False):
    ...
    # 文件处理器（带缓冲）
    LOG_FILE = os.path.join(current_dir, f"jazzcash_auto_payout_{os.getpid()}.log")
...
# JazzCash API配置参数（与 EasyPaisa 共用凭证，通过端口区分）
...
class JazzCashAutoPayout:
    def __init__(self, name):
        ...
        self.REDIS_KEYS = {
            ...
            'jazzcash_release_time': 'jazzcash_release_time',     # JazzCash账号释放时间Hash
            'jazzcash_failures': 'jazzcash_failures',             # JazzCash失败记录Hash（统一存储）
            'grab_df_prefix': 'grab_df_',                           # 订单锁前缀（共享）
            'jazzcash_account_lock_prefix': 'jazzcash_account_lock:', # 账号锁前缀（JazzCash独立）
            'payment_id_lock_prefix': 'payment_id_lock:',           # Payment ID锁前缀（共享）
            'payment_id_failed_prefix': 'payment_id_failed_jazzcash:',       # Payment ID失败锁前缀（简单冷却标记）
            # ========== 以下配置直接使用 EasyPaisa 的 key（因为抢同一个订单表，必须共享） ==========
            # - 订单冷却期Hash表: 'easypaisa_order_cooldown'
            # - 订单冷却期配置: 'easypaisa_order_cooldown_config'
    ...
    def log_complete_transaction(self, order_data: dict, account_info: dict, api_request: dict,
        ...
        if is_pakistan_mobile and ifsc_code.lower() == 'jazzcash':
            ...
            to_bank_code = "JAZZCASH"
            to_bank_name = "JazzCash"
        else:
            transfer_type = "跨行转账到银行卡"
            operation_type = "transfer_cross_bank"
            # 使用转换后的数字 bankCode（与转账时保持一致）
    ...
    def main(self):
        """主处理循环 - 基于induspay_v2框架，整合EasyPaisa数据库轮询业务逻辑"""
        try:
            # 生成新的trace_id
            trace_id_filter.trace_id = f"{os.getpid()}_{uuid.uuid4()}"

            # 🚨 优先检查紧急停机状态（使用 EasyPaisa 的配置，全局控制）
            ...
            if emergency_stop is not None and emergency_stop != b"0" and emergency_stop != "0":
                self.logger.warning(f"⚠️ 检测到全局紧急停机状态（值：{emergency_stop}），停止所有订单处理")
                time.sleep(10)  # 等待5秒后再次检查
                return
        ...
        try:
            ...
            if response and response.status_code == 200:
                try:
                    ...
                    self.logger.error(f"JazzCash API查询响应解析失败: {e}")
                    return None
            else:
