...
def setup_high_performance_logging(use_async=False):
    ...
    # 文件处理器（带缓冲）
    LOG_FILE = os.path.join(current_dir, f"easypaisa_monitor_{os.getpid()}.log")
...
class AutoPayoutMonitor:
    ...
    def _init_easypaisa_config(self):
        ...
        self.limits_cache_ttl = 300  # 限额缓存5分钟（已用额度实时变化，需要频繁刷新）
        ...
        self.REDIS_KEYS = {
            ...
            'easypaisa_limits_hash': 'easypaisa_limits_hash',       # 限额数据Hash（仅自动代付监控系统使用）
            # 添加auto_payout.py的锁键配置
            'easypaisa_account_lock_prefix': 'easypaisa_account_lock:', # 账号锁前缀（auto_payout.py使用）
    ...
    async def get_online_easypaisa_accounts(self):
        ...
        try:
            ...
            if not hash_data:
                self.logger.info(f"{self.hash_key}中没有已登录的EasyPaisa账号")
                return []
            ...
            for account_id, login_data_str in hash_data.items():
                try:
                    ...
                    account_info = {
                        ...
                        'account_name': f'EasyPaisa_{phone}',  # 使用手机号显示
                        'phone': phone,  # 从数据库查询的手机号
                        'easypaisa_id': phone,  # EasyPaisa使用手机号
            ...
            self.logger.error(f"获取在线EasyPaisa账号列表失败: {e}")
            return []
    ...
    async def check_account_health(self, login_data):
        ...
        if not payment_info or not payment_info.get('phone'):
            return {
                ...
                'account_name': f"EasyPaisa_{payment_id}",
                'phone': 'UNKNOWN',
                'easypaisa_id': payment_id,
    ...
    async def call_easypaisa_balance_api(self, login_data):
        """调用EasyPaisa余额查询API（按照EasyPaisa v1.0文档格式）"""
        import base64
        import hashlib
        ...
        try:
            ...
            if not account_accno:
                self.logger.error(f"payment_id {payment_id} 缺少account_accno字段")
                return {
                    'success': False,
                    'error': f'payment_id {payment_id} 缺少account_accno字段，无法查询余额',
                    'response_time': 0
                }
            ...
            self.logger.info(f"调用EasyPaisa余额API: account_id={phone}, api_url={self.api_url}, proxies={proxies}")
    ...
    async def call_easypaisa_limits_api(self, login_data):
        ...
        try:
            ...
            self.logger.info(f"调用EasyPaisa限额API: payment_id={payment_id}, account_id={phone}, api_url={self.api_url}")
            ...
            if result.get('code') == 200:
                # 根据EasyPaisa文档，限额在 data.body 字段
                data = result.get('data', {})
                body = data.get('body', {}) if isinstance(data, dict) else {}
                
                # 提取限额信息
                limits_info = {
                    'creditDaily': body.get('creditDaily', '0'),
                    'creditMonthly': body.get('creditMonthly', '0'),
                    'creditYearly': body.get('creditYearly', '0'),
                    'debitDaily': body.get('debitDaily', '0'),
                    'debitMonthly': body.get('debitMonthly', '0'),
                    'debitYearly': body.get('debitYearly', '0'),
                ...
                self.logger.info(f"限额查询成功: payment_id={payment_id}, 借记日限额={limits_info['debitDaily']}/{limits_info['debitDailyThreshold']}")
                return {
                    'success': True,
                    'limits': limits_info,
                    'response_time': response_time,
            ...
            self.logger.error(f"调用EasyPaisa限额API异常: payment_id={payment_id if 'payment_id' in locals() else 'unknown'}, error={e}")
            import traceback
            self.logger.error(f"异常堆栈: {traceback.format_exc()}")
            return {
                'success': False,
    ...
    async def update_redis_cache(self, status_info):
        ...
        try:
            ...
            if status_info['is_online'] and status_info['status'] == 'online':
                ...
                # 更新状态缓存（包含余额）
                status_key = f"{self.REDIS_KEYS['easypaisa_status_prefix']}{account_id}"
                status_data = {
    ...
    async def save_limits_to_redis(self, payment_id, limits_data):
        ...
        try:
            ...
            self.logger.info(f"限额数据已保存到Redis: payment_id={payment_id}, 借记日限额={limits_record['debitDaily']}/{limits_record['debitDailyThreshold']}, 月限额={limits_record['debitMonthly']}/{limits_record['debitMonthlyThreshold']}")
            return True
    ...
    def get_limits_from_redis(self, payment_id):
        ...
        try:
            ...
            if limits_json:
                ...
                self.logger.debug(f"从Redis获取限额: payment_id={payment_id}, 借记日限额={limits_data.get('debitDaily')}/{limits_data.get('debitDailyThreshold')}")
                return limits_data
            else:
                self.logger.debug(f"Redis中无限额数据: payment_id={payment_id}")
                return None
    ...
    def get_all_limits_from_redis(self):
        ...
        try:
            ...
            # 获取Hash中所有数据
            all_limits = self.redis.hgetall(limits_hash_key)
            
            if not all_limits:
                self.logger.debug("Redis中无任何限额数据")
                return {}
            
            # 解析所有限额数据
            result = {}
            for payment_id, limits_json in all_limits.items():
                try:
                    if isinstance(payment_id, bytes):
                        payment_id = payment_id.decode('utf-8')
                    if isinstance(limits_json, bytes):
                        limits_json = limits_json.decode('utf-8')
                    
                    result[payment_id] = json.loads(limits_json)
                except Exception as e:
                    self.logger.error(f"解析限额数据失败: payment_id={payment_id}, error={e}")
                    continue
            
            self.logger.info(f"从Redis获取到 {len(result)} 个账号的限额数据")
            return result
    ...
    async def generate_monitor_report(self, all_status):
        ...
        try:
            ...
            # 输出报告
            self.logger.info(f"=== EasyPaisa自动代付账号监控报告 ===")
    ...
    async def run_easypaisa_monitor_check(self):
        ...
        try:
            ...
            self.logger.info(f"{self.hash_key}中的EasyPaisa账号状态检查完成")
    ...
    def remove_account_completely(self, account_id, reason="未知原因"):
        ...
        try:
            ...
            self.logger.info(f"删除详情: hash删除{hash_deleted}项, zset删除{zset_deleted}项, balance删除{balance_deleted}项, limits删除{limits_deleted}项")
    ...
    def refresh_job_account_fields(self, payment_data):
        ...
        try:
            ...
            self.logger.error(f"刷新账号 {payment_id} 的 job 字段失败，hash 解析异常: {e}")
            return False
        ...
        if account_accno and login_data.get('account_accno') != account_accno:
            ...
            changed = True
        if account_iban and login_data.get('account_iban') != account_iban:
        ...
        self.redis.hset(self.hash_key, payment_id, simplejson.dumps(login_data))
        return True
    ...
    async def process_single_member_async(self, member: bytes) -> bool:
        ...
        try:
            ...
            # EasyPaisa 监控业务逻辑
            return await self._process_easypaisa_monitor(member)
    ...
    async def _process_easypaisa_monitor(self, member: bytes) -> bool:
        """处理 EasyPaisa 自动代付监控业务逻辑"""
        _id = member.decode()
        _lock = None
        
        try:
            # 设置 trace_id
            trace_id_filter.trace_id = f"easypaisa_autopayout_monitor_{os.getpid()}"
            ...
            self.logger.info(f"进程 {os.getpid()} 处理 EasyPaisa 自动代付监控 member[{member}], hash_key: {self.hash_key}")

            # 尝试获取锁
            self.logger.info(f"{_id}, EasyPaisa 自动代付监控尝试获取锁")
            _lock = self.get_lock(_id)
            if not _lock:
                self.logger.warning(f"{_id}, EasyPaisa 自动代付监控未获取到锁！")
                return False

            self.logger.info(f"{_id}, EasyPaisa 自动代付监控获取到锁: {_lock}")
            ...
            if not self.check_payment_status_in_db(payment_id):
                ...
                self.cleanup_missing_db_payment(payment_id, phone=login_data.get('phone'))
            ...
            self.logger.info(f"开始执行账号 {_id} 的 EasyPaisa 自动代付监控检查 (未被auto_payout.py锁定)")
            ...
            self.logger.error(f"EasyPaisa 自动代付监控业务逻辑处理异常: {e}")
            self.logger.error(f"详细错误: {traceback.format_exc()}")
            return False
        finally:
            # 删除锁
...
if __name__ == "__main__":
    try:
        ...
        monitor = AutoPayoutMonitor("ep_monitor")  # 🔥 可以改为：ep_monitor, easypaisa_auto 等
        # 定期输出日志统计（如果使用异步处理器）
