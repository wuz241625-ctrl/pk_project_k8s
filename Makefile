...
.PHONY: d7pay-preflight d7pay-render-config d7pay-apply-config d7pay-build-app d7pay-deploy d7pay-deploy-service d7pay-healthcheck d7pay-rollback
...
d7pay-apply-config:
	@bash $(D7PAY_DIR)/scripts/apply-config.sh
...
d7pay-deploy-service:
	...
	@echo "d7pay-deploy-service 现在只做 D7pay 配置应用；$(SERVICE) 的构建和滚动发布请走现有发布脚本。"
	@bash $(D7PAY_DIR)/scripts/apply-config.sh
...
d7pay-deploy-api d7pay-deploy-admin d7pay-deploy-merchant d7pay-deploy-admin-h5 d7pay-deploy-merchant-h5 d7pay-deploy-apkdownload:
	@echo "$@ 现在只做 D7pay 配置应用；$(patsubst d7pay-deploy-%,%,$@) 的构建和滚动发布请走现有发布脚本。"
	@bash $(D7PAY_DIR)/scripts/apply-config.sh
